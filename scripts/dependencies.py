...
def _run_command(command: typing.Tuple[str, ...]) -> int:
    ...
    :return: the integer representing the exit code from the subprocess
    """
    return subprocess.run(command, check=False, stdout=sys.stdout, stderr=subprocess.STDOUT).returncode
